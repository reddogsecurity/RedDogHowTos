...
.DESCRIPTION
  ...
  - AD-Assessment-Report-*.xlsx: Multi-tab Excel workbook with all assessment data
  - Executive-Brief-*.html: Executive-friendly 2-page summary (print to PDF)
...
.EXAMPLE
  ...
  Run full assessment and generate visual diagrams (Privileged Access Map, GPO Topology, Trust Map, App Grants)
  Requires Graphviz for PNG rendering: https://graphviz.org/download/
#>
...
try {
    Import-Module (Join-Path $ModulePath "AD-Collector.psm1") -Force -ErrorAction Stop
    Write-Host "  [OK] Loaded: AD-Collector" -ForegroundColor Green
} catch {
    Write-Warning "Failed to load AD-Collector module"
}

try {
    Import-Module (Join-Path $ModulePath "Entra-Collector.psm1") -Force -ErrorAction Stop
    Write-Host "  [OK] Loaded: Entra-Collector" -ForegroundColor Green
} catch {
    Write-Warning "Failed to load Entra-Collector module"
}

if ($GenerateDiagrams) {
    try {
        Import-Module (Join-Path $ModulePath "GraphGenerator.psm1") -Force -ErrorAction Stop
        Write-Host "  [OK] Loaded: GraphGenerator" -ForegroundColor Green
    } catch {
        Write-Warning "Failed to load GraphGenerator module"
    }
}

if ($IncludeEntra) {
    try {
        Import-Module (Join-Path $ModulePath "ConditionalAccess-Analyzer.psm1") -Force -ErrorAction Stop
        Write-Host "  [OK] Loaded: ConditionalAccess-Analyzer" -ForegroundColor Green
    } catch {
        Write-Warning "Failed to load ConditionalAccess-Analyzer module"
    }
}

if ($CompareWith) {
    try {
        Import-Module (Join-Path $ModulePath "Historical-TrendAnalyzer.psm1") -Force -ErrorAction Stop
        Write-Host "  [OK] Loaded: Historical-TrendAnalyzer" -ForegroundColor Green
    } catch {
        Write-Warning "Failed to load Historical-TrendAnalyzer module"
    }
}

# Always import MITRE mapper for enhanced findings categorization
try {
...
if ($IncludeThreatHunting) {
    try {
        Import-Module (Join-Path $ModulePath "ThreatHunting-Collector.psm1") -Force -ErrorAction Stop
        Write-Host "  [OK] Loaded: ThreatHunting-Collector" -ForegroundColor Green
    } catch {
        Write-Warning "ThreatHunting-Collector module failed to load: $_"
    }
    try {
        Import-Module (Join-Path $ModulePath "ThreatHunting-Analyzer.psm1") -Force -ErrorAction Stop
        Write-Host "  [OK] Loaded: ThreatHunting-Analyzer" -ForegroundColor Green
    } catch {
        Write-Warning "ThreatHunting-Analyzer module failed to load: $_"
    }
}

if ($IncludeMimecast) {
    try {
        Import-Module (Join-Path $ModulePath "Mimecast-Collector.psm1") -Force -ErrorAction Stop
        Write-Host "  [OK] Loaded: Mimecast-Collector" -ForegroundColor Green
    } catch {
        Write-Warning "Mimecast-Collector module failed to load: $_"
    }
    try {
        Import-Module (Join-Path $ModulePath "Mimecast-Analyzer.psm1") -Force -ErrorAction Stop
        Write-Host "  [OK] Loaded: Mimecast-Analyzer" -ForegroundColor Green
    } catch {
        Write-Warning "Mimecast-Analyzer module failed to load: $_"
    }
}
...
# Note: AD Collection is now handled by the AD-Collector.psm1 module
# Note: Entra Collection is now handled by the Entra-Collector.psm1 module

# --- COMPREHENSIVE ANALYSIS ENGINE ---
function Invoke-InventoryAnalysis {
    param(
    ...
    # --- Pre-calculate risk data sets for analysis ---
    # Password never expires
    $pwdNeverExpires = $adUsers | Where-Object { 
        $_.Enabled -eq 'True' -and $_.PasswordNeverExpires -eq 'True' 
    }
    
    # Delegation risks - users
    $delegUsers = $adUsers | Where-Object { 
        $_.TrustedForDelegation -eq 'True' -or $_.TrustedToAuthForDelegation -eq 'True' 
    }
    
    # Unconstrained delegation - computers
    $unconstrainedComps = $adComps | Where-Object { 
        $_.TrustedForDelegation -eq 'True' 
    }
    
    # Oversized groups
    $bigGroups = $adGroups | Where-Object { 
        $_.MemberCount -and [int]$_.MemberCount -ge 500 
    }
    ...
    # 4) AD: Unconstrained delegation on computers (ENHANCED: exclude DCs which legitimately have this flag)
    $dcNames = @()
    try { $dcNames = (Get-ADDomainController -Filter * -ErrorAction SilentlyContinue).Name } catch {}
    $unconstrainedNonDC = $unconstrainedComps | Where-Object { $dcNames -notcontains $_.Name }
    if ($unconstrainedNonDC.Count -gt 0) {
        $compDetail = ($unconstrainedNonDC | Select-Object -First 5 | ForEach-Object { "$($_.Name) ($($_.OperatingSystem))" }) -join '; '
        $remedy = Get-RemediationGuidance 'UnconstrainedDelegation'
        $findings.Add([pscustomobject]@{
            Area='AD Computers'
            Finding="$($unconstrainedNonDC.Count) non-DC computers with Unconstrained Delegation (examples: $compDetail)"
            Severity='High'
    ...
    # 6) AD: SPN surface (Kerberoast risk) - ENHANCED: escalate to Critical if any SPN account has AdminCount=1
    if ($adSpn.Count -gt 0) {
        # Check for privileged SPN accounts (AdminCount=1) - load from ad-users data
        $privilegedSpns = @()
        foreach ($spn in $adSpn) {
            $matchedUser = $adUsers | Where-Object { $_.SamAccountName -eq $spn.SamAccountName -and $_.AdminCount -eq '1' }
            if ($matchedUser) { $privilegedSpns += $spn }
        }

        if ($privilegedSpns.Count -gt 0) {
            # Critical: privileged service accounts are Kerberoastable
            $remedy = Get-RemediationGuidance 'SPNAccounts'
            $findings.Add([pscustomobject]@{
                Area='AD SPNs'
                Finding="CRITICAL: $($privilegedSpns.Count) privileged accounts (AdminCount=1) are Kerberoastable (examples: $(($privilegedSpns | Select-Object -First 3 -ExpandProperty SamAccountName) -join ', '))"
                Severity='High'
                Evidence='ad-spn-accounts'
                Impact='CRITICAL: Privileged service accounts with SPNs can be Kerberoasted - cracking the hash grants domain admin equivalent access.'
                RemediationSteps=$remedy.Steps
                Reference=$remedy.Reference
                EstimatedEffort=$remedy.Effort
                Category=$remedy.Category
    ...
    if ($authMethods.Count -gt 0) {
        ...
        if ($noMFA.Count -gt 0) {
            $remedy = Get-RemediationGuidance 'NoMFA'
            $findings.Add([pscustomobject]@{
                Area='Zero Trust'
                Finding="$($noMFA.Count) users without MFA registered"
                Severity='High'
                Evidence='entra-authmethods'
                Impact=$remedy.Impact
                RemediationSteps=$remedy.Steps
                Reference=$remedy.Reference
                EstimatedEffort=$remedy.Effort
    ...
    if ($signins.Count -gt 0) {
        ...
        if ($legacy.Count -gt 0) {
            ...
            $remedy = Get-RemediationGuidance 'LegacyAuth'
            $findings.Add([pscustomobject]@{
                Area='Legacy Auth'
    ...
    if ($riskySps.Count -gt 0) {
        $remedy = Get-RemediationGuidance 'RiskyServicePrincipals'
        $findings.Add([pscustomobject]@{
            Area='Service Principals'
            Finding="$($riskySps.Count) enterprise apps need review (owners/permissions)"
            Severity='Medium'
            Evidence='entra-serviceprincipals'
            Impact=$remedy.Impact
            RemediationSteps=$remedy.Steps
            Reference=$remedy.Reference
            EstimatedEffort=$remedy.Effort
    ...
    if ($oauth2 -and $oauth2.Count -gt 0) {
        ...
        if ($adminConsent.Count -gt 0) {
            $remedy = Get-RemediationGuidance 'OAuthPermissions'
            $findings.Add([pscustomobject]@{
                Area='OAuth Permissions'
                Finding="$($adminConsent.Count) admin-consented OAuth grants (review for excessive permissions)"
                Severity='Medium'
                Evidence='entra-oauth2-grants'
                Impact=$remedy.Impact
                RemediationSteps=$remedy.Steps
                Reference=$remedy.Reference
                EstimatedEffort=$remedy.Effort
    ...
    if ($spCreds.Count -gt 0) {
        # Secrets expiring soon (under 30 days)
        ...
        if ($expiringSoon.Count -gt 0) {
            $findings.Add([pscustomobject]@{
                ...
                Finding="$($expiringSoon.Count) service principal credentials expiring in less than 30 days"; 
                Severity='High'; 
                Evidence='entra-sp-credentials'
            })
        }

        # Long-lived secrets (over 1 year lifetime)
        ...
        if ($longLived.Count -gt 0) {
            $findings.Add([pscustomobject]@{
                Area='Service Principal Security'; 
                Finding="$($longLived.Count) service principals with credentials valid for more than 1 year"; 
                Severity='Medium'; 
                Evidence='entra-sp-credentials'
            })
        }
    ...
    if ($adPwdPolicy) {
        ...
        if ([int]$adPwdPolicy.MaxPasswordAge.Days -gt 90) {
            $findings.Add([pscustomobject]@{
                Area='Password Policy'; 
                Finding="Password max age is $($adPwdPolicy.MaxPasswordAge.Days) days (recommend 90 days or less for privileged accounts)"; 
                Severity='Low'; 
                Evidence='ad-default-pwd-policy'
            })
        }
    ...
    if ($adTrusts.Count -gt 0) {
        ...
        if ($externalTrusts.Count -gt 0) {
            $findings.Add([pscustomobject]@{
                Area='Domain Trusts'; 
                Finding="$($externalTrusts.Count) external/forest trusts present - review for least privilege and SID filtering"; 
                Severity='Medium'; 
                Evidence='ad-trusts'
            })
        }
    ...
    # === THREAT HUNTING RULES (22-27) — requires -IncludeThreatHunting ===

    if ($IncludeThreatHunting) {
        # Load thresholds if provided
        $thresholdsPath = $ThresholdsPath
        if (-not $thresholdsPath) {
            $thresholdsPath = Join-Path $PSScriptRoot "config\alert-thresholds.json"
        }

        # Invoke threat hunting data collection (if not already run)
        if (Get-Command Invoke-ThreatHuntingCollection -ErrorAction SilentlyContinue) {
            Write-Host "`nRunning threat hunting collection..." -ForegroundColor Cyan
            try {
                Invoke-ThreatHuntingCollection -OutputFolder $OutputFolder -Timestamp $NowTag
            } catch {
                Write-Warning "Threat hunting collection failed: $_"
            }
        }

        # Load threat hunting findings from analyzer
        if (Get-Command Invoke-ThreatHuntAnalysis -ErrorAction SilentlyContinue) {
            try {
                $threatHuntFindings = Invoke-ThreatHuntAnalysis -OutputFolder $OutputFolder -NowTag $NowTag -ThresholdsPath $thresholdsPath
                foreach ($tf in $threatHuntFindings) {
                    $findings.Add($tf)
                }
                Write-Host "  [OK] Threat hunt rules added $($threatHuntFindings.Count) findings" -ForegroundColor Green
            } catch {
                Write-Warning "Threat hunting analysis failed: $_"
            }
        }

        # Mimecast email security collection + analysis
        if ($IncludeMimecast -and (Get-Command Invoke-MimecastCollection -ErrorAction SilentlyContinue)) {
            try {
                $mcConfigPath = if ($MimecastConfigPath) { $MimecastConfigPath } else { Join-Path $PSScriptRoot "config\mimecast-config.json" }
                Invoke-MimecastCollection -OutputFolder $OutputFolder -Timestamp $NowTag -ConfigPath $mcConfigPath | Out-Null
            } catch {
                Write-Warning "Mimecast collection failed: $_"
            }
        }
        if ($IncludeMimecast -and (Get-Command Invoke-MimecastAnalysis -ErrorAction SilentlyContinue)) {
            try {
                $mcFindings = Invoke-MimecastAnalysis -DataPath $OutputFolder -ThresholdsPath $thresholdsPath
                foreach ($mf in $mcFindings) {
                    $findings.Add($mf)
                }
                Write-Host "  [OK] Mimecast analysis added $($mcFindings.Count) findings" -ForegroundColor Green
            } catch {
                Write-Warning "Mimecast analysis failed: $_"
            }
        }

        # 22) DCSync rights (direct rule in case ThreatHunting-Analyzer not loaded)
        $dcsyncFile = if ($NowTag) { Join-Path $OutputFolder "threat-dcsync-rights-$NowTag.json" } else { Get-LatestFile -Pattern 'threat-dcsync-rights-*.json' -Folder $OutputFolder }
        if ($dcsyncFile -and (Test-Path $dcsyncFile)) {
            $dcsyncData = Get-Content $dcsyncFile | ConvertFrom-Json
            $illegalDCSync = @($dcsyncData | Where-Object { -not $_.IsLegitimate })
            if ($illegalDCSync.Count -gt 0 -and -not (Get-Command Invoke-ThreatHuntAnalysis -ErrorAction SilentlyContinue)) {
                foreach ($entry in $illegalDCSync) {
                    $findings.Add([pscustomobject]@{
                        ...
                        Impact='CRITICAL: This principal can dump all domain credentials using DCSync without touching DC disk.'
                        RemediationSteps='1. URGENT: Remove replication right|2. Reset krbtgt password twice|3. Audit credential use since right was added'
        ...
        # 23) AdminSDHolder (direct rule fallback)
        $adminSDFile = if ($NowTag) { Join-Path $OutputFolder "threat-adminsdholder-acl-$NowTag.json" } else { Get-LatestFile -Pattern 'threat-adminsdholder-acl-*.json' -Folder $OutputFolder }
        if ($adminSDFile -and (Test-Path $adminSDFile)) {
            $adminSDData = Get-Content $adminSDFile | ConvertFrom-Json
            $adminSDAbuse = @($adminSDData | Where-Object { -not $_.IsLegitimate -and $_.IsHighRisk })
            if ($adminSDAbuse.Count -gt 0 -and -not (Get-Command Invoke-ThreatHuntAnalysis -ErrorAction SilentlyContinue)) {
                foreach ($entry in $adminSDAbuse) {
                    $findings.Add([pscustomobject]@{
                        ...
                        Impact='CRITICAL: ACEs propagated by SDProp to all protected accounts every 60 min.'
                        RemediationSteps='1. URGENT: Remove ACE from AdminSDHolder|2. Audit who added it|3. Investigate the principal'
        ...
        # 24) AS-REP roastable accounts (direct rule fallback)
        $asrepFile = if ($NowTag) { Join-Path $OutputFolder "threat-asrep-accounts-$NowTag.json" } else { Get-LatestFile -Pattern 'threat-asrep-accounts-*.json' -Folder $OutputFolder }
        if ($asrepFile -and (Test-Path $asrepFile)) {
            $asrepData = Get-Content $asrepFile | ConvertFrom-Json
            $enabledAsrep = @($asrepData | Where-Object { $_.Enabled -eq $true })
            $kpis.ASREPRoastableAccounts = $enabledAsrep.Count
        }

        # 25) Stale privileged accounts (cross-reference stale users against privileged groups)
        $privConfigPath = Join-Path $PSScriptRoot "config\privileged-config.json"
        if (Test-Path $privConfigPath) {
            try {
                $privConfig = Get-Content $privConfigPath | ConvertFrom-Json
                $tier0Groups = $privConfig.privilegedADGroups.tier0
                $privGroupsFile = if ($NowTag) { Join-Path $OutputFolder "ad-privileged-groups-$NowTag.json" } else { Get-LatestFile -Pattern 'ad-privileged-groups-*.json' -Folder $OutputFolder }
                if ($privGroupsFile -and (Test-Path $privGroupsFile)) {
                    $privGroupData = Get-Content $privGroupsFile | ConvertFrom-Json
                    $allPrivMembers = @($privGroupData | Where-Object { $tier0Groups -contains $_.Group } | ForEach-Object { $_.Members } | Where-Object { $_ })
                    $privSamNames = @($allPrivMembers | ForEach-Object { if ($_.SamAccountName) { $_.SamAccountName } })
                    $stalePriv = $stale | Where-Object { $privSamNames -contains $_.SamAccountName }
                    if ($stalePriv.Count -gt 0) {
                        $findings.Add([pscustomobject]@{
                            Area='Threat Hunt: Privileged Access'
                            Finding="CRITICAL: $($stalePriv.Count) stale PRIVILEGED accounts inactive >90 days (examples: $(($stalePriv | Select-Object -First 3 -ExpandProperty SamAccountName) -join ', '))"
                            Severity='Critical'
                            Evidence='ad-users,ad-privileged-groups'
                            Impact='Stale privileged accounts are high-value attack targets — credentials may be compromised without detection, with no legitimate owner to notice suspicious activity.'
                            RemediationSteps='1. URGENT: Disable these privileged accounts immediately|2. Remove from all privileged groups|3. Investigate last activity for signs of compromise|4. Implement automated privileged account lifecycle policy|5. Require re-certification of privileged access every 90 days'
                            Reference='https://learn.microsoft.com/en-us/windows-server/identity/ad-ds/plan/security-best-practices/implementing-least-privilege-administrative-models'
                        ...
                        $kpis.StalePrivilegedAccounts = $stalePriv.Count
                    }
                }
            } catch {
                Write-Warning "Stale privileged account check failed: $_"
            }
        }
    }
    ...
    # Helper function: Convert DN to CN (extract readable group name)
    function Convert-DNToCN {
        param([string]$dn)
        if (-not $dn) { return $dn }
    ...
    foreach ($kv in $refinedClusters.GetEnumerator() | Where-Object { $_.Value.Count -ge 3 }) {
        ...
        # Convert DNs to readable CNs for display
        $groupDNs = $kv.Key -split '\|'
        $groupNames = ($groupDNs | ForEach-Object { Convert-DNToCN $_ }) -join ', '
    ...
    if ($unlink.Count -gt 0) {
        $remedy = Get-RemediationGuidance 'UnlinkedGPOs'
        $findings.Add([pscustomobject]@{
            Area='GPOs'
            Finding="$($unlink.Count) GPOs have no links (candidates to retire or migrate to Intune)"
            Severity='Low'
            Evidence='ad-gpo-links'
            Impact=$remedy.Impact
            RemediationSteps=$remedy.Steps
            Reference=$remedy.Reference
            EstimatedEffort=$remedy.Effort
    ...
    if ($aceIssues -gt 0) {
        $remedy = Get-RemediationGuidance 'OUDelegation'
        $findings.Add([pscustomobject]@{
            Area='OU Delegation'
            Finding="$aceIssues potentially risky OU ACEs for non-admin principals"
            Severity='Medium'
            Evidence='ad-ou-acls'
            Impact=$remedy.Impact
            RemediationSteps=$remedy.Steps
    ...
    $rbacHtml = if ($rbacCandidates.Count -gt 0) {
        $rbacCandidates | Select-Object RoleName,UserCount,SourceGroupNames,ClusteringMethod | ConvertTo-Html -Fragment
...
$analysis = Invoke-InventoryAnalysis -OutputFolder $OutputFolder -NowTag $now

# === ENRICH FINDINGS WITH MITRE ATT&CK MAPPINGS ===
Write-Host "" # Blank line
...
if (Get-Command Add-MITREMapping -ErrorAction SilentlyContinue) {
    try {
        ...
        # Generate MITRE category reports
        $mitreReport = New-MITRECategoryReport `
            -EnrichedFindings $enrichedFindings `
            -OutputFolder $OutputFolder `
            -Timestamp $now

        # Update analysis with enriched findings
        $analysis.Findings = $enrichedFindings

        # Re-export risk findings with MITRE data
    ...
    # Still export the findings without MITRE data
...
# === CONDITIONAL ACCESS GAP ANALYSIS (if Entra data collected) ===
$caGapAnalysis = $null
if ($IncludeEntra) {
    try {
        $caJson = Get-LatestFile -Pattern "entra-conditionalaccess-*.json" -Folder $OutputFolder
        $usersJson = Get-LatestFile -Pattern "entra-users-*.csv" -Folder $OutputFolder
        
        if ($caJson) {
            Write-Host "" # Blank line
            $caGapAnalysis = Invoke-CAGapAnalysis `
                -OutputFolder $OutputFolder `
                -Timestamp $now `
                -CAPoliciesJson $caJson `
                -EntraUsersCsv $usersJson
        }
    } catch {
...
# Calculate severity counts for HTML summary
...
# Build HTML with enhanced styling (dark mode, print CSS, responsive)
$htmlHead = @"
<style>
    /* === LIGHT MODE (Default) === */
    :root {
        --bg-primary: #f5f5f5;
        --bg-secondary: #ffffff;
        --bg-meta: linear-gradient(135deg, #e8f4fc 0%, #d4e9f7 100%);
        --bg-info: #fff4e6;
        --text-primary: #333;
        --text-secondary: #666;
        --border-color: #ddd;
    ...
    @media (prefers-color-scheme: dark) {
        :root {
            --bg-primary: #1e1e1e;
            --bg-secondary: #2d2d2d;
            --bg-meta: linear-gradient(135deg, #1a3a52 0%, #2d4a5e 100%);
            --bg-info: #3d3520;
            --text-primary: #e0e0e0;
    ...
    .playbook-table td[contenteditable="true"] { 
        border: 1px dashed #ccc; 
        min-width: 120px; 
    }
    .playbook-table td[contenteditable="true"]:focus { 
    ...
    @media print {
        ...
        h1 { color: #0078d4 !important; page-break-before: always; }
        h2 { page-break-after: avoid; }
        table { page-break-inside: avoid; }
        .meta, .kpi-box, .info-box, .executive-summary { 
...
$htmlBody = @"
<button id="darkModeBtn" class="dark-mode-toggle" onclick="toggleDarkMode()">🌙 Dark Mode</button>
...
<div class="container">
    ...
    <div class="meta">
        ...
        <strong>Entra Included:</strong> $($meta.IncludeEntra)<br>
    ...
    <div class="executive-summary">
        ...
        <div class="summary-grid">
            <div class="summary-card">
                ...
                <div class="value">$($analysis.Findings.Count)</div>
                <div class="label">Security Issues</div>
            </div>
            <div class="summary-card">
                <h3>High Severity</h3>
                <div class="value" style="color: #d13438;">$highFindings</div>
                <div class="label">Immediate Action</div>
            </div>
            <div class="summary-card">
                <h3>Medium Severity</h3>
                <div class="value" style="color: #ff8c00;">$medFindings</div>
                <div class="label">Plan Remediation</div>
            </div>
            <div class="summary-card">
                <h3>Low Severity</h3>
                <div class="value" style="color: #107c10;">$lowFindings</div>
                <div class="label">Hygiene Items</div>
            </div>
            <div class="summary-card">
                <h3>AD Users</h3>
                <div class="value">$($analysis.KPIs.UsersAD)</div>
                <div class="label">Active Directory</div>
            </div>
            <div class="summary-card">
                <h3>Entra Users</h3>
                <div class="value">$($analysis.KPIs.UsersEntra)</div>
                <div class="label">Azure AD/Entra</div>
            </div>
        </div>
    </div>
...
<ol>
    ...
    <li><strong>RBAC Implementation:</strong> Use the seed roles in $($analysis.RbacCsv) to design your Entra RBAC model</li>
    <li><strong>GPO Migration:</strong> Review $($analysis.GpoCsv) and plan Intune baseline migration</li>
...
<ul>
    ...
    <li><strong>GPO Modernization:</strong> ##GPO_CSV##</li>
    <li><strong>KPIs (JSON):</strong> ##KPI_JSON##</li>
</ul>

    <p style="margin-top: 40px; color: var(--text-secondary); font-size: 0.9em; border-top: 1px solid var(--border-color); padding-top: 20px;">
    <em>Generated by Black-Box Security Assessment Script v2.3 | All data collected in read-only mode</em><br>
    <em>Report generated: ##COLLECTED_AT##</em>
    </p>
</div>
"@
...
$htmlBody = $htmlBody.Replace('##GPO_CSV##', $analysis.GpoCsv)
$htmlBody = $htmlBody.Replace('##KPI_JSON##', $analysis.KpiJson)
$htmlBody = $htmlBody.Replace('##COLLECTED_AT##', $meta.CollectedAt)

$reportTitle = "BlackBox Security Assessment $now"
$html = ConvertTo-Html -Head $htmlHead -Body $htmlBody -Title $reportTitle
...
if ($caGapAnalysis) {
    Write-Host "  - CA Gap Analysis:    $($caGapAnalysis.GapCsvPath)" -ForegroundColor Gray
    Write-Host "  - CA Coverage Stats:  $($caGapAnalysis.CoverageJsonPath)" -ForegroundColor Gray
}
if ($mitreReport) {
    Write-Host "  - MITRE Categories:   $($mitreReport.SecurityCategoryCsvPath)" -ForegroundColor Gray
    Write-Host "  - MITRE Tactics:      $($mitreReport.TacticCsvPath)" -ForegroundColor Gray
}
if ($trendAnalysis) {
    Write-Host "  - Trend Analysis:     $($trendAnalysis.TrendCsvPath)" -ForegroundColor Gray
}
...
if ($GenerateDiagrams) {
    try {
        Write-Host "" # Blank line
        $diagramResults = Invoke-DiagramGeneration -OutputFolder $OutputFolder -Timestamp $now
        
        if ($diagramResults) {
            Write-Host "`nGenerated Diagrams:" -ForegroundColor Cyan
            if ($diagramResults.PrivilegedAccess) {
                Write-Host "  - Privileged Access Map:" -ForegroundColor White
                Write-Host "    $($diagramResults.PrivilegedAccess.Mermaid)" -ForegroundColor Gray
                if ($diagramResults.PrivilegedAccess.PNG) {
                    Write-Host "    $($diagramResults.PrivilegedAccess.PNG)" -ForegroundColor Gray
                }
            }
            if ($diagramResults.GPOTopology) {
                Write-Host "  - GPO Topology:" -ForegroundColor White
                Write-Host "    $($diagramResults.GPOTopology.Mermaid)" -ForegroundColor Gray
            }
            if ($diagramResults.TrustMap) {
                Write-Host "  - Trust Map:" -ForegroundColor White
                Write-Host "    $($diagramResults.TrustMap.Mermaid)" -ForegroundColor Gray
            }
            if ($diagramResults.AppGrant) {
                Write-Host "  - App and Grant Views:" -ForegroundColor White
                Write-Host "    $($diagramResults.AppGrant.Mermaid)" -ForegroundColor Gray
            }
        }
    } catch {
        Write-Warning "Diagram generation encountered errors: $($_.Exception.Message)"
    }
}

# === HISTORICAL TREND ANALYSIS (if previous assessment provided) ===
$trendAnalysis = $null
if ($CompareWith -and (Test-Path $CompareWith)) {
    try {
        Write-Host "" # Blank line
        $trendAnalysis = Invoke-TrendAnalysis `
            -CurrentFolder $OutputFolder `
            -PreviousFolder $CompareWith `
            -Timestamp $now
    } catch {
        Write-Warning "Trend analysis failed: $($_.Exception.Message)"
    }
}
...
Write-Host "Generating Stakeholder Reports" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
...
try {
    Write-Host "Creating Excel workbook..." -ForegroundColor Gray
    $excelScriptPath = Join-Path $PSScriptRoot "Export-ExcelReport.ps1"
    if (Test-Path $excelScriptPath) {
        $excelPath = & $excelScriptPath -OutputFolder $OutputFolder -Timestamp $now
        if ($excelPath -and (Test-Path $excelPath)) {
            Write-Host "[OK] Excel workbook created" -ForegroundColor Green
        }
    } else {
        Write-Host "[INFO] Excel export script not found (optional)" -ForegroundColor DarkGray
    }
} catch {
    Write-Warning "Excel export failed: $($_.Exception.Message)"
}

# Generate Executive Brief (HTML)
try {
    Write-Host "Creating executive brief..." -ForegroundColor Gray
    $briefScriptPath = Join-Path $PSScriptRoot "Export-ExecutiveBrief.ps1"
    if (Test-Path $briefScriptPath) {
        $briefPath = & $briefScriptPath -OutputFolder $OutputFolder -Timestamp $now
        if ($briefPath -and (Test-Path $briefPath)) {
            Write-Host "[OK] Executive brief created" -ForegroundColor Green
        }
    } else {
        Write-Host "[INFO] Executive brief script not found (optional)" -ForegroundColor DarkGray
    }
} catch {
    Write-Warning "Executive brief export failed: $($_.Exception.Message)"
}

if ($excelPath -or $briefPath) {
    Write-Host "`nStakeholder Reports Generated:" -ForegroundColor Cyan
    if ($excelPath) {
        Write-Host "  - Excel Workbook: $excelPath" -ForegroundColor White
    }
    if ($briefPath) {
        Write-Host "  - Executive Brief: $briefPath" -ForegroundColor White
    }
    Write-Host "" # Blank line
}
...
if ($GenerateDiagrams) {
    Write-Host "  6. Review visual diagrams for privileged access paths and relationships" -ForegroundColor Gray
}
if ($excelPath -or $briefPath) {
    $stepNum = if ($GenerateDiagrams) { 7 } else { 6 }
    if ($excelPath) {
        Write-Host "  $stepNum. Share Excel workbook with technical teams for detailed analysis" -ForegroundColor Gray
        $stepNum++
    }
    if ($briefPath) {
        Write-Host "  $stepNum. Print executive brief to PDF for leadership distribution" -ForegroundColor Gray
    }
}
