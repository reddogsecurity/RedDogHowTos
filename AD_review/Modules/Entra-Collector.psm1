...
function Invoke-EntraCollection {
    ...
    Write-Host "  [OK] Collected $($users.Count) users" -ForegroundColor Green
    ...
    Write-Host "Enumerating service principals and enterprise apps..." -ForegroundColor Gray
    ...
    $spLimit = [Math]::Min(500, $sps.Count)
    Write-Host "  Processing $spLimit service principals..." -ForegroundColor DarkGray
    
    $spCredentials = @()
    $spSubset = $sps | Select-Object -First $spLimit
    $processedSP = 0
    $spProgressInterval = [Math]::Max(1, [Math]::Floor($spLimit / 10))
    
    foreach ($sp in $spSubset) {
        try {
            # Get full service principal with credentials included
            $spFull = Get-MgServicePrincipal -ServicePrincipalId $sp.Id -Property "id,displayName,passwordCredentials,keyCredentials" -ErrorAction SilentlyContinue
            ...
            if ($spFull.PasswordCredentials) {
                foreach ($secret in $spFull.PasswordCredentials) {
            ...
            if ($spFull.KeyCredentials) {
                foreach ($cert in $spFull.KeyCredentials) {
        ...
        $processedSP++
        if ($processedSP % $spProgressInterval -eq 0) {
            $percent = [Math]::Round(($processedSP / $spLimit) * 100)
            Write-Host "    Progress: $percent% ($processedSP of $spLimit SPs)" -ForegroundColor DarkGray
        }
    }
    
    Write-Host "  [OK] Collected credentials for $($spCredentials.Count) service principals" -ForegroundColor Green
    ...
    # MFA / Authentication Methods (with progress indicator)
    Write-Host "Collecting authentication methods (MFA coverage)..." -ForegroundColor Gray
    $authLimit = [Math]::Min(500, $users.Count)
    Write-Host "  Processing $authLimit users for MFA status (this may take a few minutes)..." -ForegroundColor DarkGray
    
    try {
        $authMethods = @()
        $userSubset = $users | Select-Object -First $authLimit
        $processedCount = 0
        $progressInterval = [Math]::Max(1, [Math]::Floor($authLimit / 10))  # Update every 10%
        
        foreach ($user in $userSubset) {
            try {
            ...
            if ($processedCount % $progressInterval -eq 0) {
                $percent = [Math]::Round(($processedCount / $authLimit) * 100)
                Write-Host "    Progress: $percent% ($processedCount of $authLimit users)" -ForegroundColor DarkGray
            }
        }
        
        Write-Host "  [OK] Collected MFA status for $($authMethods.Count) users" -ForegroundColor Green
