...
function Invoke-ADCollection {
    ...
    # Users, groups, computers (paged with progress)
    Write-Host "Enumerating users..." -ForegroundColor Gray
    $userCount = (Get-ADUser -Filter * -ResultSetSize 1 | Measure-Object).Count
    Write-Host "  Found $userCount users - collecting details..." -ForegroundColor DarkGray
    ...
    Write-Host "  [OK] Collected $($groups.Count) groups" -ForegroundColor Green
    ...
    Write-Host "  [OK] Collected $($computers.Count) computers" -ForegroundColor Green
    ...
    # Protected Users group — accounts NOT in this group are more vulnerable to
    # pass-the-hash and Kerberos delegation abuse
    Write-Host "Collecting Protected Users group membership..." -ForegroundColor Gray
    Get-ProtectedUsersGroup -OutputFolder $OutputFolder -Timestamp $Timestamp
...
function Get-ProtectedUsersGroup {
    <#
    .SYNOPSIS
    Collects membership of the Protected Users security group.
    Privileged accounts NOT in this group are more vulnerable to credential theft.
    #>
    [CmdletBinding()]
    param(
    ...
    try {
        $group = Get-ADGroup -Filter "Name -eq 'Protected Users'" -ErrorAction SilentlyContinue
        if ($group) {
            ...
            } | ConvertTo-Json -Depth 5 | Out-File (Join-Path $OutputFolder "threat-protected-users-$Timestamp.json") -Force
            Write-Host "  [OK] Protected Users group: $(@($members).Count) members" -ForegroundColor DarkGray
        } else {
            @{ Group = 'Protected Users'; MemberCount = 0; Members = @(); Note = 'Group not found - may need to create it' } |
                ConvertTo-Json | Out-File (Join-Path $OutputFolder "threat-protected-users-$Timestamp.json") -Force
            Write-Warning "Protected Users group not found in domain"
        }
    } catch {
        Write-Warning "Protected Users collection failed: $_"
    }
}

Export-ModuleMember -Function Invoke-ADCollection, Get-ProtectedUsersGroup
