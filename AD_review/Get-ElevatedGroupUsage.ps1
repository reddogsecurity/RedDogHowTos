...
function Get-GroupUsageMetrics {
    ...
    try {
        ...
        if ($userMembers.Count -gt 0) {
            $activeRatio = if ($userMembers.Count -gt 0) { 
                ($activeMemberCount / $userMembers.Count) * 100 
            } else { 0 }
...
foreach ($group in $groupsToAnalyze) {
    ...
    $usage = Get-GroupUsageMetrics -Group $group -Recursive $IncludeNestedMembers
    ...
    if ($usage) {
        ...
        $analysisResults += [PSCustomObject]@{
            ...
            } elseif ($usage.DaysSinceLastActivity -ge 180) {
                "CRITICAL: No activity in 180+ days - Review for removal"
            } elseif ($usage.DaysSinceLastActivity -ge 90) {
...
$summary = [PSCustomObject]@{
    ...
    InactiveGroups = ($analysisResults | Where-Object { $_.DaysSinceLastActivity -ge $DaysInactive }).Count
...
<body>
    ...
    $(if ($criticalGroups.Count -gt 0) {
        "<div class='alert alert-critical'>
            <strong>⚠️ CRITICAL:</strong> $($criticalGroups.Count) groups have no activity in 180+ days and require immediate review!
        </div>"
    })
    
    $(if ($emptyGroups.Count -gt 0) {
        "<div class='alert alert-warning'>
            <strong>⚠️ WARNING:</strong> $($emptyGroups.Count) empty groups detected - consider cleanup.
        </div>"
    })
    ...
    $(if ($criticalGroups.Count -gt 0) {
        "<h2>⚠️ Critical Risk Groups (180+ Days Inactive)</h2>
        <table>
            <tr>
                <th>Group Name</th>
                <th>Elevated</th>
    ...
    <div class="alert alert-info">
        <strong>ℹ️ Usage Score Calculation:</strong><br>
        Usage Score (0-100%) is calculated based on:<br>
        • Percentage of active members (logged in within $DaysInactive days)<br>
        • Recent activity bonus (20 points for activity within 30 days, 10 points for 60 days)<br>
        • Higher scores indicate more frequently used groups
    </div>
...
if ($mostUsedGroups.Count -gt 0) {
    Write-Host "`n[✓] Top 3 Most Used Groups:" -ForegroundColor Green
