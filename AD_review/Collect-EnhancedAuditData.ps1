...
function Get-GPOData {
    ...
    try {
        ...
        Write-Host "[OK] GPO data collected: $($gpos.Count) GPOs" -ForegroundColor Green
...
function Get-OUData {
    ...
    try {
        ...
        Write-Host "[OK] OU data collected: $($ous.Count) OUs" -ForegroundColor Green
...
function Get-DomainTrustData {
    ...
    try {
        ...
        Write-Host "[OK] Trust data collected: $($trusts.Count) trusts" -ForegroundColor Green
...
function Get-DomainData {
    ...
    try {
        ...
        Write-Host "[OK] Domain data collected: $($domains.Count) domains" -ForegroundColor Green
...
function Get-ServicePrincipalData {
    param([string]$OutputPath)
    ...
    Write-Host "[OK] Sample service principal data created" -ForegroundColor Green
}

# Function to create sample OAuth scope data
function Get-OAuthScopeData {
    param([string]$OutputPath)
    ...
    Write-Host "[OK] Sample OAuth scope data created" -ForegroundColor Green
}
...
function Get-ConditionalAccessData {
    ...
    Write-Host "[OK] Sample Conditional Access data created" -ForegroundColor Green
}
...
foreach ($dataType in $existingDataTypes) {
    ...
    if (-not $existingFile) {
        ...
        Write-Host "[OK] Found existing $($dataType.Name) data: $($existingFile.Name)" -ForegroundColor Green
    }
}

# Collect new data types
Get-GPOData -OutputPath (Join-Path $OutputFolder "gpo-data-$Timestamp.csv")
Get-OUData -OutputPath (Join-Path $OutputFolder "ou-data-$Timestamp.csv")
Get-DomainTrustData -OutputPath (Join-Path $OutputFolder "domain-trusts-$Timestamp.csv")
Get-DomainData -OutputPath (Join-Path $OutputFolder "domain-data-$Timestamp.csv")
Get-ServicePrincipalData -OutputPath (Join-Path $OutputFolder "service-principals-$Timestamp.json")
Get-OAuthScopeData -OutputPath (Join-Path $OutputFolder "oauth-scopes-$Timestamp.json")
Get-ConditionalAccessData -OutputPath (Join-Path $OutputFolder "ca-policies-$Timestamp.json")
