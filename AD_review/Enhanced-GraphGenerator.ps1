...
function New-RiskOverviewDiagram {
    ...
    [void]$mmd.AppendLine('  subgraph High["[H] High Risk ($($highRiskNodes.Count) items)"]')
    ...
    [void]$mmd.AppendLine('  subgraph Medium["[M] Medium Risk ($($mediumRiskNodes.Count) items)"]')
    ...
    [void]$mmd.AppendLine('  end')
    [void]$mmd.AppendLine('  subgraph Low["[L] Low Risk ($($lowRiskNodes.Count) items)"]')
